...
void Tracking::Track()
{
    ...
    else
    {
        ...
        if(!mbOnlyTracking)
        {
            ...
            if(!bOK) {
                cout << "--- [SLAM LOG] VISUAL TRACKING LOST ---" << endl;
                cout << "Reason: The camera moved too fast (motion blur) OR pointed at a textureless surface (blank wall/face)." << endl;
                cout << "Matches found: " << mnMatchesInliers << " (Required: 25 during init, 10 after init)" << endl;
        ...
        else if (mState == OK)
        {
            if (mSensor == System::IMU_MONOCULAR || mSensor == System::IMU_STEREO || mSensor == System::IMU_RGBD)
            {
                ...
                // --- [DIAG] Why are we reseting? ---
                bool bImuReady  = pCurrentMap->isImuInitialized();
                bool bBA2Ready  = pCurrentMap->GetIniertialBA2();
                int  nKFs       = pCurrentMap->KeyFramesInMap();
                int  nMPs       = pCurrentMap->MapPointsInMap();
                cout << "[DIAG] TrackLost->ResetCheck: ImuInit=" << bImuReady
                     << " | BA2done=" << bBA2Ready
                     << " | KFs=" << nKFs
                     << " | MapPts=" << nMPs
                     << " | inliers_at_fail=" << mnMatchesInliers
                     << " | frameId=" << mCurrentFrame.mnId
                     << " | sinceReloc=" << (int)(mCurrentFrame.mnId - mnLastRelocFrameId) << endl;
                if(!bImuReady)
                    cout << "[DIAG] RESET REASON: IMU NOT YET INITIALIZED (need more motion/time)" << endl;
                else if(!bBA2Ready)
                    cout << "[DIAG] RESET REASON: IMU BA2 (global inertial BA) NOT YET COMPLETE" << endl;
        ...
        {
            int nKFs = pCurrentMap->KeyFramesInMap();
            bool bImuInit = pCurrentMap->isImuInitialized();
            cout << "[DIAG] State=LOST: KFs=" << nKFs
                 << " | ImuInit=" << bImuInit
                 << " | inliers=" << mnMatchesInliers
                 << " | frameId=" << mCurrentFrame.mnId << endl;

            if(nKFs<=10)
            {
                cout << "[DIAG] RESET PATH: Too few KFs (" << nKFs << " <= 10), hard reset" << endl;
                mpSystem->ResetActiveMap();
                return;
            }
            if (mSensor == System::IMU_MONOCULAR || mSensor == System::IMU_STEREO || mSensor == System::IMU_RGBD)
                if (!bImuInit)
                {
                    cout << "[DIAG] RESET PATH: LOST before IMU init, KFs=" << nKFs << ", hard reset" << endl;
                    Verbose::PrintMess("Track lost before IMU initialisation, reseting...", Verbose::VERBOSITY_QUIET);
                    mpSystem->ResetActiveMap();
                    return;
                }

            cout << "[DIAG] RESET PATH: CreateMapInAtlas() (soft map switch)" << endl;
            CreateMapInAtlas();
...
bool Tracking::TrackReferenceKeyFrame()
{
    ...
    if(nmatches<15)
    {
        cout << "[DIAG] TrackReferenceKF FAIL: BoW matches=" << nmatches << " < 15. RefKF_id=" << mpReferenceKF->mnId
             << " MapPoints_in_RefKF=" << mpReferenceKF->GetMapPoints().size() << endl;
        return false;
    }
...
bool Tracking::TrackWithMotionModel()
{
    ...
    if(nmatches<20)
    {
        cout << "[DIAG] TrackMotionModel FAIL: ProjectionSearch nmatches=" << nmatches << " < 20 (th=" << th << ", 2*th=" << 2*th << ")"
             << " | mbVelocity=" << mbVelocity
             << " | LocalMapPts=" << mvpLocalMapPoints.size()
             << " | FrameFeatures=" << mCurrentFrame.N << endl;
        if (mSensor == System::IMU_MONOCULAR || mSensor == System::IMU_STEREO || mSensor == System::IMU_RGBD)
            return true;
        else
            return false;
...
bool Tracking::TrackLocalMap()
{
    ...
    if (!mpAtlas->isImuInitialized())
    {
        cout << "[DIAG] TrackLocalMap Optimizer: PoseOptimization (IMU not init)" << endl;
        Optimizer::PoseOptimization(&mCurrentFrame);
    }
    else
    {
        if(mCurrentFrame.mnId<=mnLastRelocFrameId+mnFramesToResetIMU)
        {
            cout << "[DIAG] TrackLocalMap Optimizer: PoseOptimization (post-reloc window, frameId="
        ...
        else
        {
            ...
            {
                cout << "[DIAG] TrackLocalMap Optimizer: PoseInertialOptimizationLastFrame (mapUpdated=false)" << endl;
                Verbose::PrintMess("TLM: PoseInertialOptimizationLastFrame ", Verbose::VERBOSITY_DEBUG);
                inliers = Optimizer::PoseInertialOptimizationLastFrame(&mCurrentFrame); // , !mpLastKeyFrame->GetMap()->GetIniertialBA1());
            }
            else
            {
                cout << "[DIAG] TrackLocalMap Optimizer: PoseInertialOptimizationLastKeyFrame (mapUpdated=true)" << endl;
    ...
    // --- [DIAG] TrackLocalMap detailed stats ---
    {
        int nTotalMPs = 0, nOutliers = 0;
        for(int i=0; i<mCurrentFrame.N; i++)
            if(mCurrentFrame.mvpMapPoints[i]) { nTotalMPs++; if(mCurrentFrame.mvbOutlier[i]) nOutliers++; }
        bool bImuInit = mpAtlas->isImuInitialized();
        bool bBA2Done = mpAtlas->GetCurrentMap()->GetIniertialBA2();
        cout << "[DIAG] TrackLocalMap: inliers=" << mnMatchesInliers
             << " | totalMPs_in_frame=" << nTotalMPs << " | outliers_after_PO=" << nOutliers
             << " | localMapPts=" << mvpLocalMapPoints.size()
             << " | localKFs=" << mvpLocalKeyFrames.size()
             << " | ImuInit=" << bImuInit << " | BA2=" << bBA2Done
             << " | state=" << (int)mState
             << " | sinceReloc=" << (int)(mCurrentFrame.mnId - mnLastRelocFrameId) << endl;
    }
    ...
    {
        cout << "[DIAG] TrackLocalMap FAIL: post-reloc window (" << (int)(mCurrentFrame.mnId-mnLastRelocFrameId)
             << " frames since reloc), inliers=" << mnMatchesInliers << " < 25" << endl;
        return false;
    }
    ...
    if (mSensor == System::IMU_MONOCULAR)
    {
        if((mnMatchesInliers<10 && mpAtlas->isImuInitialized())||(mnMatchesInliers<25 && !mpAtlas->isImuInitialized()))
        {
            cout << "[DIAG] TrackLocalMap FAIL IMU_MONO: inliers=" << mnMatchesInliers
                 << (mpAtlas->isImuInitialized() ? " < 10 (IMU ready, strict)" : " < 25 (IMU not init yet)") << endl;
            return false;
        }
        else
            return true;
    ...
    else if (mSensor == System::IMU_STEREO || mSensor == System::IMU_RGBD)
    {
        ...
        {
            cout << "[DIAG] TrackLocalMap FAIL IMU_STEREO/RGBD: inliers=" << mnMatchesInliers << " < 15" << endl;
            return false;
        }
        else
            return true;
...
void Tracking::SearchLocalPoints()
{
    ...
    // --- [SLAM LOG] SearchLocalPoints stats before matching ---
    // Count keypoints in this frame that already have a MapPoint assigned (prior to SearchByProjection)
    int nAlreadyMatched = 0;
    for(int i = 0; i < mCurrentFrame.N; i++)
        if(mCurrentFrame.mvpMapPoints[i] && !mCurrentFrame.mvbOutlier[i])
            nAlreadyMatched++;
    cout << "[DIAG] SearchLocalPoints: localMapPts=" << mvpLocalMapPoints.size()
         << " | inFrustum(nToMatch)=" << nToMatch
         << " | alreadyMatchedInFrame=" << nAlreadyMatched
         << " | totalFrameKPs=" << mCurrentFrame.N << endl;
    ...
    if(nToMatch>0)
    {
        ...
        // --- [SLAM LOG] SearchLocalPoints result ---
        cout << "[DIAG] SearchByProjection: th=" << th
             << " | newMatchesAdded=" << matches
             << " | totalCandidateProjected=" << nToMatch
             << " | matchRate=" << (nToMatch > 0 ? (100.0f * matches / nToMatch) : 0.0f) << "%"
             << " | reason_low_match="
             << (matches < 10 ?
                   (nToMatch < 20 ? "TOO_FEW_POINTS_IN_FRUSTUM(scene_textureless_or_cam_rotated)" :
                                   "DESCRIPTOR_MISMATCH(motion_blur_or_illumination_change)") : "OK")
             << endl;
    }
    else
    {
        cout << "[DIAG] SearchLocalPoints: nToMatch=0 -> NO points in camera frustum! "
             << "Cause: map too small, camera pointing away, or map reset just happened." << endl;
    }
}
